...
The library is versioned in **lockstep** with the compiler: `teko_std` 0.6.0 is the library
`teko` 0.6.0 compiles, and the two are released together.
...
compiler tag it is locked to: `teko` v0.6.0 carries `MC_VERSION` 0.15.23 and is built by
...
2. its dependency, `teko` (`[deps] teko = "0.6.0"`, the pinned version above), registered and
