...
The library is versioned in **lockstep** with the compiler: `teko_std` 0.4.2 is the library
`teko` 0.4.2 compiles, and the two are released together.
...
compiler tag it is locked to: `teko` v0.4.2 carries `MC_VERSION` 0.15.22 and is built by
...
2. its dependency, `teko` (`[deps] teko = "0.4.2"`, the pinned version above), registered and
